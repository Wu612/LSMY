...
//	
	...
	$scope.dd=function(){
		if($scope.userPhone&&$scope.userPassword){
		console.log($scope.userPhone);
	    console.log($scope.userPassword);
			$(".wraper button").addClass("button-positive")
		}else{
			 $(".wraper button").removeClass("button-positive"); 
		}
	}
...
.controller('buyCarCtrl',function($scope,$timeout){
	...
	for(var i=0;i<listObj.length;i++){
	  ...
	  Div.innerHTML= '<div class="wait_wrap">'
	     +'<ul class="box_wrap">'
       	+'<li class="checkbox-all"><label class="checkbox"><input type="checkbox" class="select"/></label>	</li>'
 ...
 function ishasCheck(pid){
  ...
  function delProduct(id){
  	...
  	for(var i=0;i<listObj.lenght;i++){
  		if(listObj[i].pid==id){
  		  listObj.splice(i,1);
  		}
  	}
   ...
   var total=$(".fixde_footer .font span");
